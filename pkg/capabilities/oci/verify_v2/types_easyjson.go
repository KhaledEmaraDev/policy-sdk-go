		...
		case "certificate_chain":
			if in.IsNull() {
				...
				if out.CertificateChain == nil {
					if !in.IsDelim(']') {
						out.CertificateChain = make([]capabilities.RuneString, 0, 4)
					} else {
						out.CertificateChain = []capabilities.RuneString{}
					}
				} else {
					out.CertificateChain = (out.CertificateChain)[:0]
				}
				for !in.IsDelim(']') {
					var v18 capabilities.RuneString
					(v18).UnmarshalEasyJSON(in)
					out.CertificateChain = append(out.CertificateChain, v18)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjson6601e8cdEncodeGithubComKubewardenPolicySdkGoPkgCapabilitiesOciVerifyV28(out *jwriter.Writer, in sigstoreCertificateVerify) {
	...
	}
	{
		...
		if in.CertificateChain == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			...
			out.RawByte('[')
			for v20, v21 := range in.CertificateChain {
				if v20 > 0 {
					out.RawByte(',')
				}
	...
	}
	{
		...
		if in.Annotations == nil && (out.Flags&jwriter.NilMapAsEmpty) == 0 {
			...
			out.RawByte('{')
			v22First := true
			for v22Name, v22Value := range in.Annotations {
				if v22First {
					v22First = false
				} else {
					out.RawByte(',')
