...
type v2VerifyTestCase struct {
	...
	checkIsTrustedFunc func(host VerifyV2Host, request easyjson.Marshaler) (bool, error)
}
	...
	for description, testCase := range map[string]v2VerifyTestCase{
		...
		"Certificate": {
			request: sigstoreCertificateVerify{
				...
				CertificateChain:   []cap.RuneString{"certificate1", "certificate2"},
...
func CheckPubKeysImageTrusted(host VerifyV2Host, request easyjson.Marshaler) (bool, error) {
	requestPubKeys := request.(sigstorePubKeysVerify)
	res, err := host.VerifyPubKeysImage(requestPubKeys.Image, requestPubKeys.PubKeys, requestPubKeys.Annotations)
	if err != nil {
		return false, err
	}
...
func CheckKeylessExactMatchTrusted(host VerifyV2Host, request easyjson.Marshaler) (bool, error) {
	requestKeylessExactMatch := request.(sigstoreKeylessVerifyExact)
	res, err := host.VerifyKeylessExactMatch(requestKeylessExactMatch.Image, requestKeylessExactMatch.Keyless, requestKeylessExactMatch.Annotations)
	if err != nil {
		return false, err
	}
	return res.IsTrusted, nil
...
func CheckKeylessPrefixMatchTrusted(host VerifyV2Host, request easyjson.Marshaler) (bool, error) {
	requestKeylessPrefixMatch := request.(sigstoreKeylessPrefixVerify)
	res, err := host.VerifyKeylessPrefixMatch(requestKeylessPrefixMatch.Image, requestKeylessPrefixMatch.KeylessPrefix, requestKeylessPrefixMatch.Annotations)
	if err != nil {
		return false, err
	}
	return res.IsTrusted, nil
...
func CheckKeylessGithubActionsTrusted(host VerifyV2Host, request easyjson.Marshaler) (bool, error) {
	requestKeylessGithubActions := request.(sigstoreGithubActionsVerify)
	res, err := host.VerifyKeylessGithubActions(requestKeylessGithubActions.Image, requestKeylessGithubActions.Owner, requestKeylessGithubActions.Repo, requestKeylessGithubActions.Annotations)
	if err != nil {
		return false, err
	}
	return res.IsTrusted, nil
...
func CheckCertificateTrusted(host VerifyV2Host, request easyjson.Marshaler) (bool, error) {
	requestCertificate := request.(sigstoreCertificateVerify)

	res, err := host.VerifyCertificate(requestCertificate.Image, requestCertificate.Certificate, requestCertificate.CertificateChain, requestCertificate.RequireRekorBundle, requestCertificate.Annotations)
	if err != nil {
		return false, err
	}
	return res.IsTrusted, nil
