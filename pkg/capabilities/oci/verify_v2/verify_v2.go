...
func (h *VerifyV2Host) VerifyPubKeysImage(image string, pubKeys []string, annotations map[string]string) (oci.VerificationResponse, error) {
	...
	return h.Verify(requestObj, oci.V2)
}
...
// * annotations: annotations that must have been provided by all signers when they signed the OCI artifact
func (h *VerifyV2Host) VerifyKeylessExactMatch(image string, keyless []oci.KeylessInfo, annotations map[string]string) (oci.VerificationResponse, error) {
...
// * `annotations` - annotations that must have been provided by all signers when they signed the OCI artifact
func (h *VerifyV2Host) VerifyKeylessPrefixMatch(image string, keylessPrefix []KeylessPrefixInfo, annotations map[string]string) (oci.VerificationResponse, error) {
...
// * `annotations` - annotations that must have been provided by all signers when they signed the OCI artifact
func (h *VerifyV2Host) VerifyKeylessGithubActions(image string, owner string, repo string, annotations map[string]string) (oci.VerificationResponse, error) {
...
//   - `annotations` - annotations that must have been provided by all signers when they signed the OCI artifact
func (h *VerifyV2Host) VerifyCertificate(image string, certificate cap.RuneString, certificateChain []cap.RuneString, requireRekorBundle bool, annotations map[string]string) (oci.VerificationResponse, error) {
