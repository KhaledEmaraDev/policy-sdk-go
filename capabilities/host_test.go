...
func TestV1v1IsCertificateTrusted(t *testing.T) {
	ctrl := gomock.NewController(t)
	m := mock_capabilities.NewMockWapcClient(ctrl)
	...
	requestPayload, err := easyjson.Marshal(requestObj)
	if err != nil {
		t.Fatalf("cannot serialize request object: %v", err)
	}
	...
	m.
		EXPECT().
		HostCall("kubewarden", "crypto", "v1/is_certificate_trusted", requestPayload).
		Return(verificationPayload, nil).
		Times(1)
	...
	res, err := host.VerifyCert(cert, nil, "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !res {
